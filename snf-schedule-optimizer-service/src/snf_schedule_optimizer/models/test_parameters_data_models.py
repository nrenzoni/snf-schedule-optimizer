...
class FacilityParameters:
    ...
    team_consistency_weight: float = (
        5.0  # Weight for minimizing team changes (soft constraint)
    )
...
class PerShiftStressTestParameters:
    ...
    admission_surge_factor: (
        float  # Multiplier for unexpected admissions (e.g., 1.0 to 1.5)
    )
    high_acuity_mix_increase: (
        float  # Percentage increase in residents with high-NTA/high-GG scores
    )
