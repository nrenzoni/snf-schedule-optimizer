...
class IIdObfuscator(abc.ABC):
    @abc.abstractmethod
    def encode(self, internal_id: int) -> str:
        """Converts internal DB integer to public string."""
        pass
...
class IdObfuscator(IIdObfuscator):
    """
    Infrastructure service to handle ID obfuscation.
    Keeps the 'secret' alphabet and logic out of the domain.
    """
    ...
    def __init__(
        ...
        # In production, load alphabet/salt from environment variables
        if alphabet is None:
            alphabet = ALPHABET
        self._sqids = Sqids(
            alphabet=alphabet, min_length=min_length, blocklist=DEFAULT_BLOCKLIST
        )
