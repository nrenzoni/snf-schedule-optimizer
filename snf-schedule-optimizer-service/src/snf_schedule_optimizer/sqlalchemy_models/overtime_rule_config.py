...
class OvertimeRuleConfig(SQLABase):
    ...
    applicable_job_titles: Mapped[list[str] | None] = mapped_column(
        ARRAY(String), nullable=True
    )
    union_contract_id: Mapped[str | None] = mapped_column(
        String, nullable=True
    )  # e.g., 'Local_123'

    # --- OvertimeTrigger Fields (Thresholds) ---
    daily_threshold: Mapped[float | None] = mapped_column(
        Float, nullable=True
    )  # e.g., 8.0
    weekly_threshold: Mapped[float | None] = mapped_column(
        Float, nullable=True
    )  # e.g., 40.0
    ...
    work_period_start_time: Mapped[dt.time | None] = mapped_column(Time, nullable=True)

    # Daily Period Reset Time (for non-midnight day resets)
    daily_period_reset_time: Mapped[dt.time | None] = mapped_column(Time, nullable=True)

    # Specific Days of Week Trigger: Stored as array of integers (WeekDay values)
    days_of_week_trigger: Mapped[list[int] | None] = mapped_column(
