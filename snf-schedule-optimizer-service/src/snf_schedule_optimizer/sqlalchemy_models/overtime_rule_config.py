...
class OvertimeRuleConfig(SQLABase):
    ...
    )  # Description is implicitly nullable
    ...
    applicable_job_titles: Mapped[Optional[List[str]]] = mapped_column(
        ARRAY(String), nullable=True
    )
    union_contract_id: Mapped[Optional[str]] = mapped_column(
        String, nullable=True
    )  # e.g., 'Local_123'

    # --- OvertimeTrigger Fields (Thresholds) ---
    daily_threshold: Mapped[Optional[float]] = mapped_column(
        Float, nullable=True
    )  # e.g., 8.0
    weekly_threshold: Mapped[Optional[float]] = mapped_column(
        Float, nullable=True
    )  # e.g., 40.0

    # Consecutive Thresholds
    consecutive_day_threshold: Mapped[Optional[int]] = mapped_column(
        Integer, nullable=True
    )
    consecutive_hours_threshold: Mapped[Optional[float]] = mapped_column(
        Float, nullable=True
    )
    ...
    work_period_start_time: Mapped[Optional[dt.time]] = mapped_column(
        Time, nullable=True
    )

    # Daily Period Reset Time (for non-midnight day resets)
    daily_period_reset_time: Mapped[Optional[dt.time]] = mapped_column(
        Time, nullable=True
    )

    # Specific Days of Week Trigger: Stored as array of integers (WeekDay values)
    days_of_week_trigger: Mapped[Optional[List[int]]] = mapped_column(
