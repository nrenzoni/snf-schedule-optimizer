"""II.1 & II.2: Federal mandate tests for 8h consecutive RN and 24/7 licensed nurse."""
...
async def test_licensed_nurse_required_on_every_shift() -> None:
    ...
    fake_hprd = FakeHprdRequirementCalculator(
        requirements_map={(1, HprdEnforcedRole.CNA): 1.0}
    )
    ...
    assert "licensed" in details.lower()


async def test_consecutive_rn_coverage_enforced() -> None:
    ref = whenever.ZonedDateTime(2025, 1, 1, 7, tz=tz_ny)
