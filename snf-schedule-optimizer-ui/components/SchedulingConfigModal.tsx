...
export const SchedulingConfigModal: React.FC<SchedulingConfigModalProps> = ({settings, isOpen, onClose, onUpdate}) => {
    ...
    const handleSave = () => {
        ...
        (Object.keys(draftSettings) as (keyof SchedulingSettings)[]).forEach(key => {
        ...
        <div
            ...
            <div
                ...
                <div className="border-b p-4 flex justify-between items-center bg-indigo-50">
                    ...
                    <button onClick={onClose} className="text-gray-400 hover:text-gray-600 p-1 rounded-full"><X
                        size={20}/></button>
                </div>

                <div className="p-6 space-y-8 max-h-[80vh] overflow-y-auto">
                    <h4 className="text-lg font-semibold text-gray-700 border-b pb-2">Staffing & ML Integration</h4>
                    <div className="grid md:grid-cols-2 gap-8">
                        {/* 1. HPPD ML Forecast */}
                        <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg border">
                            <div>
                                <label className="text-sm font-bold text-gray-800">Factor in HPPD ML Forecast</label>
                                <p className="text-xs text-gray-500">Adjust staffing based on AI predictions</p>
                            </div>
                            <button
                                onClick={() => handleUpdate('useMLForecast', !draftSettings.useMLForecast)}
                        ...
                        <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg border">
                            <div>
                                <label className="text-sm font-bold text-gray-800">Include Call-out Buffer</label>
                                <p className="text-xs text-gray-500">Reserve extra staff for high-risk shifts</p>
                            </div>
                            <button
                                onClick={() => handleUpdate('useCalloutBuffer', !draftSettings.useCalloutBuffer)}
                        ...
                        <div
                            ...
                            <div className="flex justify-between items-center">
                                ...
                                <span
                                    className="text-sm font-mono font-bold text-indigo-600 bg-indigo-50 px-2 py-1 rounded">
                            ...
                            <input
                                ...
                                onChange={(e) => handleUpdate('bufferThreshold', parseInt(e.target.value))}
                                className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-indigo-600"
                                disabled={!draftSettings.useCalloutBuffer}
                            />
                            <p className="text-xs text-gray-500">Percentage of shift capacity to hold as reserve.</p>
                        </div>
                    </div>

                    <h4 className="text-lg font-semibold text-gray-700 border-b pb-2 pt-4">Compliance & Shift Rules</h4>
                    <div className="grid md:grid-cols-2 gap-8">
                        {/* 4. Min Rest Period */}
                        <div className="space-y-3 p-3 bg-white rounded-lg border">
                            ...
                            <input
                                ...
                                onChange={(e) => handleUpdate('minRestPeriod', parseInt(e.target.value))}
                                className="w-full border rounded-lg p-2 text-lg font-bold"
                            />
                            <p className="text-xs text-gray-500">Enforce minimum rest time between shifts
                                (Current: {draftSettings.minRestPeriod} hrs).</p>
                        </div>
                        ...
                        <div className="space-y-3 p-3 bg-white rounded-lg border">
                            ...
                            <input
                                ...
                                onChange={(e) => handleUpdate('maxShiftLength', parseFloat(e.target.value))}
                                className="w-full border rounded-lg p-2 text-lg font-bold"
                            />
                            <p className="text-xs text-gray-500">Prevent burnout/fatigue by limiting shift duration
                                (Current: {draftSettings.maxShiftLength} hrs).</p>
                        </div>
                            ...
                            <div className="flex space-x-4">
                                <div className="flex items-center space-x-2">
                                    <input
                                        ...
                                        onChange={(e) => handleUpdate('premiumWeekend', e.target.checked)}
                                        className="rounded text-indigo-600"
                                    />
                                    <label className="text-sm text-gray-700">Weekend Shifts</label>
                                </div>
                                <div className="flex items-center space-x-2">
                                    <input
                                        type="checkbox"
                                        checked={draftSettings.premiumHoliday}
                                        onChange={(e) => handleUpdate('premiumHoliday', e.target.checked)}
                                        className="rounded text-indigo-600"
                                    />
                                    <label className="text-sm text-gray-700">Holiday Shifts</label>
                                </div>
                            </div>
                            <p className="text-xs text-gray-500">Shifts matching selected criteria will be flagged for
                                premium pay/priority scheduling.</p>
                        </div>
                    </div>
                </div>

                <div className="bg-gray-50 p-4 rounded-b-xl flex justify-end space-x-3 border-t">
                    <button onClick={onClose} className="px-4 py-2 text-gray-600 hover:bg-gray-200 rounded-lg">Cancel
                    </button>
                    <button
                        onClick={handleSave}
