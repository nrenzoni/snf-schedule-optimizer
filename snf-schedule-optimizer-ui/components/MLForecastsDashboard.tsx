...
import {Activity, ArrowRight, Brain, ShieldAlert, TrendingUp, Users, X} from "lucide-react";
...
export const MLForecastsDashboard = () => {
    ...
    const forecasts: ForecastItem[] = [
        {
            ...
            details: {
                ...
                insight: 'Machine learning models detect a recurring staffing gap on Saturday nights. Recommendation: Pre-book 2 PRN shifts for Nov 23 & 30.'
            }
        },
        {
            id: 'turnover',
            ...
            details: {
                ...
                insight: 'Analysis of time-card data shows Nurse J. Doe and M. Smith have worked >60hrs for 4 consecutive weeks. Immediate schedule relief recommended.'
            }
        },
        {
            id: 'safety',
            ...
            details: {
                ...
                insight: 'No major red flags. Minor uptick in missed breaks in Wing C. Suggest auditing break-room logs.'
            }
        }
    ];
        ...
        <div className="max-w-6xl mx-auto">
            ...
            <div className="mb-8 flex items-center space-x-3">
                ...
                <div>
                    ...
                    <p className="text-gray-500">AI-driven predictions for proactive management</p>
                </div>
            </div>
                        ...
                        >
                            ...
                            <div className="flex items-start space-x-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-gray-600 mt-2 leading-snug">{item.summary}</p>
                                </div>
                            </div>
                            {activeInsight?.id === item.id && (
                ...
                <div className="lg:col-span-2">
                    {activeInsight ? (
                        <div key={activeInsight.id}
                            ...
                            <div className={`p-6 border-b ${activeInsight.bg} flex justify-between items-start`}>
                                <div>
                                    <div className="flex items-center space-x-2 mb-1">
                                        <activeInsight.icon size={18} className={activeInsight.color}/>
                                        <span
                                            className={`text-xs font-bold uppercase tracking-wider ${activeInsight.color}`}>Deep Dive Analysis</span>
                                    </div>
                                    <h3 className="text-2xl font-bold text-gray-900">{activeInsight.title}</h3>
                                </div>
                            ...
                            <div className="p-6 md:p-8 space-y-8">
                                ...
                                <div className="grid grid-cols-3 gap-4">
                                    {activeInsight.details.stats.map((stat, idx) => (
                                        <div key={idx}
                                            ...
                                            <p className={`text-xl md:text-2xl font-bold mt-1 ${stat.alert ? 'text-red-600' : 'text-gray-800'}`}>
                                                {stat.value}
                                            </p>
                                        </div>
                                    ))}
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div
                                        className="h-48 bg-gray-50 rounded-xl border border-dashed border-gray-300 flex items-end justify-between p-4 px-8">
                                        {[40, 65, 45, 80, 55, 90, 70].map((h, i) => (
                                            <div key={i}
                                                 className="w-8 bg-indigo-200 rounded-t-md hover:bg-indigo-400 transition-colors relative group"
                                                 style={{height: `${h}%`}}>
                                                <div
                                                    className="absolute -top-8 left-1/2 -translate-x-1/2 bg-gray-800 text-white text-[10px] py-1 px-2 rounded opacity-0 group-hover:opacity-100 transition">
                                                    Data Point
                                                </div>
                                            </div>
                                        ))}
                                ...
                                {/* AI Insight Box */}
                                <div
                                    className="bg-indigo-50 border border-indigo-100 p-4 rounded-xl flex items-start space-x-3">
                        ...
                        <div
                            className="h-full min-h-[400px] flex flex-col items-center justify-center text-center p-8 bg-gray-50 rounded-xl border-2 border-dashed border-gray-200 animate-in fade-in duration-500">
                            <div
                                className="w-16 h-16 bg-white rounded-full shadow-sm flex items-center justify-center mb-4 text-purple-300">
                                <Brain size={32}/>
                            </div>
                            <h3 className="text-lg font-bold text-gray-700">Select a Forecast Module</h3>
                            <p className="text-gray-400 max-w-sm mt-2">Click on one of the prediction cards on the left
                                to view detailed AI analysis and actionable insights.</p>
                        </div>
                    )}
                </div>
            </div>
