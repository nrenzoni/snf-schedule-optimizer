...
interface SimulationConfigModalProps {
    ...
    onRun: (type: ModalType | null, params: { // Function that takes the simulation parameters
        wageIncrease: number;
    ...
    if (type === 'WAGE_IMPACT') {
        ...
        content = (
            <div className="space-y-4">
                <p className="text-sm text-gray-500">Project the impact of across-the-board wage increases on the annual
                    budget.</p>
                <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Proj. Wage Increase (%)</label>
                    <input
                        type="range" min="0" max="20" step="0.5"
                        value={params.wageIncrease}
                        onChange={(e) => setParams({...params, wageIncrease: Number(e.target.value)})}
                        className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer"
                    />
                    <div className="flex justify-between text-xs text-gray-500 mt-1">
                        <span>0%</span>
                        <span className="font-bold text-indigo-600 text-lg">{params.wageIncrease}%</span>
                        <span>20%</span>
                    </div>
                </div>
            </div>
        ...
        content = (
            <div className="space-y-4">
                <p className="text-sm text-gray-500">Analyze savings by converting agency hours to internal overtime or
                    new hires.</p>
                <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Target Reduction in Agency Hours
                        (%)</label>
                    <input
                        type="number"
                        value={params.targetAgencyReduction}
                        onChange={(e) => setParams({...params, targetAgencyReduction: Number(e.target.value)})}
        ...
        content = (
            <div className="space-y-4">
                <p className="text-sm text-gray-500">Calculate HPPD and labor cost per patient day (PPD) based on census
                    shifts.</p>
                <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Projected Census Change
                        (Residents)</label>
                    <div className="flex items-center space-x-2">
                        <input
                            type="number"
                            value={params.censusChange}
                            onChange={(e) => setParams({...params, censusChange: Number(e.target.value)})}
                            className="w-full border rounded-lg p-2"
                        />
